...
const NavBar = ({ useLinks, OnlySmall, NavList, clearBG }) => {
  ...
  const windowLinks = () => {
    if (useLinks) {
      return (
        ...
        <ul className={`${OnlySmall ? 'md:hidden' : 'md:flex' } hidden list-none flex-row gap-10 select-none`}>
          {NavList.map((link) => (
            <li
            key={link.id}
            className={`${
              ...
              ? "text-white"
              : "text-secondary"
            } hover:text-white font-poppins sm:font-bold font-medium cursor-default text-[16px] sm:text-[20px] `}
        ...
        <div className={`${OnlySmall ? 'md:flex' : 'md:hidden' } select-none flex flex-1 justify-center items-center`}>
            <img src={toggleMenu ? close : menu} alt="menu" 
              className={`${toggleMenu ? 'h-[15px] w-[15px]' : 'h-[20px] w-[20px]'} object-contain cursor-default`} 
              ...
              <ul className='list-none flex justify-end items-start flex-col gap-2 py-2'>
                {NavList.map((link) => (
                  ...
                  className={`${
                    ...
                    ? "text-secondary"
                    : "text-white-200"
                  } cursor-default font-poppins font-bold text-[18px] hover:text-secondary w-full px-4 rounded-2xl`}
                  onClick={() => {
  ...
  return (
    <nav data-tauri-drag-region onContextMenu={(e)=> e.preventDefault()} className={`${styles.paddingX} ${clearBG ? 'bg-none' : 'bg-primary'} select-none cursor-default w-full py-2 fixed z-30 top-0 flex flex-row justify-between drop-shadow-[0_10px_10px_rgba(0,0,0,1)]`}>
          ...
          <ul className='list-none flex justify-end items-start flex-col gap-2 py-2'>
            {navLinks.map((link) => (
              ...
              className={`${
                ...
                ? "text-secondary"
                : "text-white-200"
              } font-poppins font-bold text-[18px] hover:text-secondary w-full px-4 rounded-2xl`}
              onClick={() => {
                setToggle(!toggle);
                setActive(link.title);
              }}
              >
                <a href={link.id_tag + link.id} className='px-3 rounded-2xl cursor-default'>
                 {link.title}
                </a>
              </li>
            ))}
      ...
      <div className="w-min flex gap-1" >
        ...
        <button onClick={() => {setMaximized(!maximized); maximized ? appWindow.unmaximize() : appWindow.maximize() }} className='contrast-0 hover:contrast-100 transition-[.3s] cursor-default'>🟠</button>
